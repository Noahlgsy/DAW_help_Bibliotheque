...
fastify.get("/Books", (request, reply) => {
    try {
        const query = database_js_1.default.prepare("SELECT Book.title, Book.author, Book.year, Book.synopsis, Gender.name AS genre FROM Book JOIN Gender ON Book.gender_id = Gender.id;");
...
fastify.post("/Books", (request, reply) => {
    try {
        const { title, author, year, synopsis, gender_id } = request.body;
        const query = database_js_1.default.prepare("INSERT INTO Book (title, author, year, synopsis, gender_id ) VALUES (?, ?, ?, ?, ?);");
...
fastify.get("/Books/search", (request, reply) => {
    try {
        ...
        const query = database_js_1.default.prepare("SELECT title, author, year, synopsis FROM Book WHERE author LIKE ? OR title LIKE ?");
...
fastify.get("/Genders", (request, reply) => {
    try {
        const query = database_js_1.default.prepare("SELECT name FROM Gender ;");
        const result = query.all();
        reply.code(200).send(result);
    }
    catch (error) {
...
fastify.post("/Genders", (request, reply) => {
    try {
        ...
        const query = database_js_1.default.prepare("INSERT INTO Gender (name) VALUES (?); ");
